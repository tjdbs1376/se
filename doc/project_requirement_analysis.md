...
flowchart LR
    ...
    subgraph MiniDrive["미니 드라이브 시스템 (System Boundary)"]
        direction TB
        UC1(["계정 인증<br/>(가입/로그인)"])
        UC2(["파일 전송<br/>(업로드/다운로드)"])
        UC3(["파일/폴더 관리<br/>(수정/삭제/복구)"])
        UC4(["파일 검색"])
        UC5(["공유 및 권한 설정"])
        UC6(["버전 관리 및 복구"])
        UC7(["외부 링크 접근"])
...
* **포함 관계**: 파일 전송 및 관리 기능은 반드시 로그인을 거쳐야 하므로 `<<include>>` 관계를 설정하였습니다.



### 2.2 유스케이스 설명서: 파일 업로드

유스케이스의 식별 정보와 상세 흐름을 나타냅니다.

| 항목          | 상세 내용                                                                                                                                                                                         |
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ID / 이름** | UC-002 / 파일 업로드 (File Upload)                                                                                                                                                                 |
| **주요 액터**   | 내부 사용자                                                                                                                                                                                        |
...
업무 수행 과정에서 필요한 객체를 식별하고 관계를 정의하였습니다. 문장 분석법을 통해 명사는 클래스로, 동사는 연산으로 매핑하였습니다.
